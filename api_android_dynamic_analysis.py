...
@request_method(['POST'])
@csrf_exempt
def api_dynamic_download(request):
    """POST - Dynamic Analysis report."""
    params = {'hash', 'file'}
    if set(request.POST) < params:
        return make_api_response(
            {'error': 'Missing Parameters'}, 422)
    path = os.path.expanduser('~') + '\\.MobSF\\downloads\\' + request.POST['file']
    try:
        with open(path, 'rb') as f:
            file_content = f.read()
        resp = HttpResponse(file_content, content_type='application/octet-stream')
        resp['Content-Disposition'] = 'attachment; filename="' + request.POST['file'] + '"'
        return resp
